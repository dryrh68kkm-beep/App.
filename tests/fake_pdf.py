...
def make_report_pdf(pages: list[FakeReport]) -> bytes:
    ...
    for spec in pages:
        ...
        th(330, 40, "บริษัท ตัวอย่าง จำกัด (ข้อมูลจำลอง)", 12)
        th(100, 62, "ข้อมูลการมาปฏิบัติงานของพนักงาน", 12)
        th(70, 84, "รหัสพนักงาน")
        if spec.employee_id:
            en(160, 84, f": {spec.employee_id}")
            th(240, 84, spec.name)
        th(70, 100, "รหัสหน่วยงาน")
        if spec.unit_code:
            en(160, 100, f": {spec.unit_code}")
        th(320, 100, "ชื่อหน่วยงาน")
        if spec.department:
            en(400, 100, f": {spec.department}")
        th(70, 116, "กลุ่มการจ่ายเงินเดือน")
        en(160, 116, ": D")
        th(320, 116, "ระหว่างวันที่")
        en(400, 116, ": 16/08/2569 - 15/09/2569")
        th(540, 130, "หน้า", 8)
        en(565, 130, str(spec.page_no), 8)
        y = 170
        for d in range(1, 26):
