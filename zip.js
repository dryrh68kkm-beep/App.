// สร้างไฟล์ ZIP แบบไม่บีบอัด (PDF บีบอัดอยู่แล้ว) รองรับชื่อไฟล์ภาษาไทย
// ทำงานในเครื่องทั้งหมด ไม่ใช้ไลบรารีภายนอก
...
/** files: [{ name: string, data: Uint8Array }] -> Uint8Array */
export function makeZip(files, now = new Date()) {
  ...
  for (const f of files) {
    ...
    local.setUint16(8, 0, true); // ไม่บีบอัด
